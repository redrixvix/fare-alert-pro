...
const CABINS = ['PREMIUM_ECONOMY', 'BUSINESS', 'FIRST'];
const ROUTES = [
...
function sanitizePrices(rawPrices, cabin) {
  const cap = ABSOLUTE_PRICE_CAP_BY_CABIN[cabin] ?? 7000;
...
(async () => {
  ...
  for (const [origin, dest] of ROUTES) {
    ...
    for (const cabin of CABINS) {
      const existing = await convex.query('prices:getPricesByRoute', { route, cabin });
      const covered = new Set(existing.map((r) => r.search_date));
      ...
      for (let offset = 0; offset < 90; offset++) {
        ...
        for (const p of prices) {
          const id = await convex.mutation('prices:insertPriceRecord', {
            route,
            cabin,
            search_date: dateStr,
            price: p.price,
        ...
        checked++;
        if (checked % 25 === 0) process.stdout.write(`\r{checked:${checked} inserted:${inserted}}`);
      }
    }
  }
  console.log(`\nDONE {checked:${checked} inserted:${inserted}}`);
})();
